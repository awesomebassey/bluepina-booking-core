...
function nights(checkIn: Date, checkOut: Date) {
  ...
  if (n <= 0 || n > 30)
    throw new BadRequestException("Stay must be 1-30 nights");
  return n;
}
...
function datesBetween(checkIn: Date, checkOut: Date) {
  ...
  return Array.from(
    { length: count },
    (_, i) => new Date(checkIn.valueOf() + i * MS_DAY),
  );
}
...
export class BookingService {
  ...
  async availability(unitId: string, fromRaw: string, toRaw: string) {
    ...
    const blocked = new Map(
      blocks.map((b) => [b.date.toISOString().slice(0, 10), b.source]),
    );
    return days.map((date) => {
      ...
      return {
        ...
        source: blocked.get(key) ?? null,
      };
    });
  }

  async availableUnits(checkInRaw: string, checkOutRaw: string) {
    const checkIn = utcDate(checkInRaw);
    const checkOut = utcDate(checkOutRaw);

    // Reuse the existing validation:
    // checkout must be after check-in and stay must be <= 30 nights.
    datesBetween(checkIn, checkOut);

    // Release any short-lived holds that have already expired
    // before deciding which units are available.
  ...
  async createHold(dto: CreateHoldDto) {
    const existing = await this.prisma.reservationHold.findUnique({
    ...
    try {
      ...
      if (
        ...
        const winner = await this.prisma.reservationHold.findUnique({
          where: { idempotencyKey: dto.idempotencyKey },
        });
        if (winner) return winner;
        throw new ConflictException(
          "One or more requested nights are no longer available",
        );
      }
      throw error;
    }
  ...
  async reapExpired() {
    ...
    await this.prisma.$transaction([
      this.prisma.reservationBlock.deleteMany({
        where: { holdId: { in: ids } },
      }),
      this.prisma.reservationHold.updateMany({
        where: { id: { in: ids } },
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        const booking = await tx.booking.create({
          data: {
            ...
            totalAmountCents: count * hold.unit.nightlyRateCents,
            currency: hold.unit.currency,
          },
        });
        ...
        await tx.reservationHold.update({
          where: { id: hold.id },
          data: { status: HoldStatus.BOOKING_CREATED },
        });
        await tx.auditEvent.create({
          data: {
            entityType: "Booking",
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (dto.status === "SUCCEEDED") {
          ...
          if (
            booking.hold.status !== HoldStatus.BOOKING_CREATED ||
            ...
            await tx.auditEvent.create({
              data: {
                ...
                metadata: {
                  ...
                  received: dto.status,
                  bookingStatus: booking.status,
                  holdStatus: booking.hold.status,
                },
              },
            });
            return {
        ...
        if (dto.status === "FAILED") {
          ...
          await tx.reservationBlock.deleteMany({
      ...
      if (
        ...
        const winner = await this.prisma.paymentEvent.findUnique({
          where: { eventId: dto.eventId },
        });
